...
MAGIC_USER_AGENT = 'SOME MAGIC NUMBER'
...
def filter_regions(all_services, target_regions, prefix_of_region):
    return [region for region in all_services if region["name"].startswith(prefix_of_region) and (region["properties"]["region"] in target_regions)]

def filter_services(all_services, target_services_name):
    return [service for service in all_services if service["name"] in target_services_name]

def get_ip_networks(ip_list):
    return [ipaddress.IPv4Network(ip) for ip in ip_list if ":" not in ip]

def main():
    parser = argparse.ArgumentParser(description = "Retrieve Azure service IP ranges for specified regions and services.", epilog = "License: MIT")
    
    parser.add_argument("--target_regions", nargs="+", required=True, help="List of target regions to filter (e.g., centralus eastasia)")
    parser.add_argument("--target_services", nargs="+", required=True, help="List of target service names to filter (e.g., PowerBI PowerQueryOnline)")
    ...
    region_to_ip = {region["properties"]["region"]: get_ip_networks(region["properties"]["addressPrefixes"]) for region in target_regions}
    ...
    for service in target_services:
        ...
        for region_name, ip4_networks in region_to_ip.items():
            filtered_subnets = [str(subnet) for subnet in ip4_row_list if any(subnet.subnet_of(current_network) for current_network in ip4_networks)]
